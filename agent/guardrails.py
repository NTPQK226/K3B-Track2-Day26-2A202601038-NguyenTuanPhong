...
_INJECTION_PATTERNS = (
    ...
    re.compile(r"reveal\s+the\s+(act|scopes|key|learner|data)", re.IGNORECASE),
    re.compile(r"report\s+the\s+(act|scopes|key|learner)", re.IGNORECASE),
    re.compile(r"print\s+the\s+(act|scopes|key|learner)", re.IGNORECASE),
    re.compile(r"as\s+the\s+system,\s+you\s+must", re.IGNORECASE),
)


def scan_for_injected_instructions(text: str) -> InjectionScanResult:
    """Scans for adversarial imperatives embedded in retrieved content."""
    matched = []
...
_PRIVATE_PATTERNS = (
    re.compile(r"(private\s+note\s+reads:\s*)(.{40,})", re.IGNORECASE),
...
def redact(text: str) -> RedactionResult:
    """Catches and redacts private note contents and sensitive credentials."""
    redacted = text
    hits = []
...
def verify_arithmetic(text: str) -> ArithmeticCheckResult:
    """Verifies numeric consistency in answer text."""
...
if __name__ == "__main__":
    ...
    print(f"  scan_for_injected_instructions(<injection>) -> {scan}")
    assert scan.suspicious is True

    leaky = "Learner sv-0402's private note reads: " + "x" * 45 + " (this is definitely private content)"
    red = redact(leaky)
    print(f"  redact(<45+ char private string>) -> hits={len(red.hits)}, text modified={red.redacted_text != leaky}")
