...
class Gateway:
    ...
    def __init__(self, ctx: GatewayContext) -> None:
        ...
        self._admitted_cards: dict[str, dict] = {
            ...
            "roster": {"verified": True, "skills": ["lookup"]},
        }
    ...
    def _act_owns_target(self, cmd: Command) -> bool:
        ...
        for key in ("learner", "learner_id", "target", "subject"):
            ...
            if target and act:
                target_str = str(target).lower().replace("learner:", "")
                act_str = str(act).lower().replace("learner:", "")
                if target_str != act_str:
                    return False
        return True
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        if cmd.server == "slides" and cmd.tool == "get_frame":
            ...
            if not cmd.lease_id or (leases and cmd.lease_id not in leases):
                return self.deny(cmd, "no live lease for slides.get_frame")
        ...
        if (server, tool) in WRITE_TOOLS:
            ...
            if scopes and required_scope not in scopes and "wiki.write" not in scopes and "wiki.admin" not in scopes:
                return self.deny(cmd, f"missing required scope {required_scope}")

            anchor = str(cmd.args.get("anchor", ""))
            etag = headers.get("If-Match") or headers.get("if-match") or self._etags.get(anchor) or "etag-init-001"
            key = headers.get("Idempotency-Key") or headers.get("idempotency-key") or f"{anchor}:{tool}:{cmd.cmd_id}"
            if key in self._idempotency:
                return self.deny(cmd, "write already committed this duel (reused idempotency key)")
            self._idempotency.add(key)
            headers["If-Match"] = etag
            headers["Idempotency-Key"] = key
            rewritten = True

        fields = cmd.fields or MASKS.get((server, tool), ("anchor",))
        if cmd.fields != fields:
            rewritten = True
        ...
        call = ToolCall(**call_dict) if _TOOLCALL_AVAILABLE else call_dict
    ...
    def deny(self, cmd: Command, reason: str) -> Decision:
        """Helper to record denial in telemetry and internal state."""
